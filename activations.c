...
void relu_backward(pLayer l){
    matrix_map(&(l->output), relu_gradient);
    matrix_copy(&(l->output), &(l->input));
}
...
void sigmoid_backward(pLayer l){
    matrix_map(&(l->output), sigmoid_gradient);
    matrix_copy(&(l->output), &(l->input));
}

void update_none_op(pLayer l){
    return ;
}
...
void softmax_forward(pLayer l){
    ...
    Matrix vector = make_matrix_zeros(1, l->output.col);
    matrix_sum(&(l->output), &vector, 0);
    matrix_div_vector(&(l->output), &vector);
}
